...
| `instagram.ts` | `fetchProfile`, `fetchMedia`, `fetchInsights`, `disconnectInstagram` + types (`InstagramProfileResponse`, `InstagramMediaResponse`, `InstagramInsightsResponse`) | Direct Instagram Graph API client. Reads the per-user long-lived token from the `creators` row; on Meta error 190 runs `ig_refresh_token` and persists via `updateCreatorToken`. Throws `Error('session_expired')` when no usable token. 15s timeout. |
