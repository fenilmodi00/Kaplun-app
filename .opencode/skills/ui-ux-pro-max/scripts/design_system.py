...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        return {
            ...
            "category": category,
            "pattern": {
                "name": reasoning.get("pattern", "Screen + Content + Primary Action"),
                "sections": "Header > Content > Primary Action",
                "cta_placement": "Sticky bottom or screen footer",
                "color_strategy": "",
                "conversion": ""
            },
            "style": {
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    # App-focused page structure defaults
    layout["Sections"] = "Header > Scrollable Content > Primary Action"
    recommendations.append("CTA Placement: sticky bottom bar or header action")

    # Add page-type specific defaults if no search results
    if not layout.get("Max Width"):
        layout["Max Width"] = "full-width (mobile-first)"
    if not layout.get("Layout"):
        layout["Layout"] = "Single-column mobile layout"
