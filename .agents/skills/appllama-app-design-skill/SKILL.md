...
1. Gather **public references** for the category and screen type: shipping apps
   in the same space (store listings, marketing pages, design write-ups, or
   screenshots the user provides). Aim for ~10 patterns you can name before
   writing UI code — no external MCP or paid research tools required.
...
6. **Study the grammar, not just the pixels.** Walking a winning flow in a
   reference app (or the user's notes), note what each step *is* — push,
   modal, sheet — and copy that consistency.
...
8. **Ship full state cycles, not the happy path.** Static-successful-state-
   ...
   inline and specific.
9. **The slop pre-flight is mechanical.** Before you hand off implementation,
   count: distinct accent hues (must be 1), distinct corner
...
- The bar: 60 fps through the hero flow on a **release build on the slowest
  device you support** — dev clients hide jank
  ([references/performance.md](references/performance.md)). The **human**
  validates motion on a physical device; if they share a screen recording,
  review it for feel and obvious glitches.
...
- Generate assets with the **best image model available to you** (any imagegen
  tool the project already uses) at the **highest quality settings**, then
  downscale to @1x/@2x/@3x. Never upscale.
...
The agent **implements** to the laws above; **you** verify on a **physical
device** with the project's **development build** (Kaplun: EAS dev APK + Expo
dev client + Metro). Do not block delivery on simulators, emulators, or agent-
driven screen recordings.

When shipping a screen, leave a short **test notes** block for the human:
which route to open, states to force (empty/loading/error), light/dark toggle,
and navigation/back paths worth tapping.

Optional checklist for your device pass:
[references/device-verification.md](references/device-verification.md).

If you paste device screenshots or a recording, treat that as the source of
truth and iterate until the reported issues are fixed.

## Definition of done, per screen (agent)

- [ ] Studied enough public reference patterns to name the layout grammar you
      adopted (no Appllama or other MCP required)
...
- [ ] Light + dark mode: tokens and styles wired; human verifies on device
- [ ] Safe areas / cutout / home indicator: layout uses insets; human verifies
- [ ] Long-content, empty, loading, and error states designed — not defaulted
- [ ] Motion: purposeful per motion laws; Reduce Motion respected in code;
      human validates feel on device (release build when judging fps)
...
| [references/device-verification.md](references/device-verification.md) | Optional human device checklist (dev APK) |
